...
export class ApplicationService {
  ...
  protected convertDateFromClient(application: IApplication): IApplication {
    const copy: IApplication = Object.assign({}, application, {
      applicationDate:
        application.applicationDate && application.applicationDate.isValid() ? application.applicationDate.toJSON() : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((application: IApplication) => {
        application.applicationDate = application.applicationDate ? moment(application.applicationDate) : undefined;
      });
    }
    return res;
  }
