...
import { getEntities, reset } from './car.reducer';
...
export const Car = (props: ICarProps) => {
  ...
  const { carList, match, loading } = props;
  return (
    <div>
      <h2 id="car-heading">
        Cars
            ...
            <Table responsive>
              <thead>
                <tr>
                  ...
                  <th className="hand" onClick={sort('manufacturedYear')}>
                    Manufactured Year <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
              ...
              <tbody>
                {carList.map((car, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>{car.price}</td>
                    <td>
                      <TextFormat type="date" value={car.manufacturedYear} format={APP_DATE_FORMAT} />
                    </td>
                    <td>{car.type ? <Link to={`car-type/${car.type.id}`}>{car.type.id}</Link> : ''}</td>
                    <td>{car.brand ? <Link to={`car-brand/${car.brand.id}`}>{car.brand.id}</Link> : ''}</td>
                    <td>{car.autopark ? <Link to={`autopark/${car.autopark.id}`}>{car.autopark.id}</Link> : ''}</td>
